...
const B = {
  forest: "#1B4332", dark: "#2D6A4F", green: "#40916C", sage: "#52B788",
  mint: "#74C69D", light: "#B7E4C7", cream: "#D8F3DC", white: "#F8FBF9",
  gold: "#C9A96E", charcoal: "#1A1A2E", text: "#2C3E2D",
};
    ...
    <>
      ...
      <style jsx global>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body { font-family: 'DM Sans', sans-serif; color: ${B.text}; background: ${B.white}; overflow-x: hidden; }

        /* NAV */
        .nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; padding: 12px 20px; transition: all 0.3s ease; }
        .nav-scrolled { background: rgba(27,67,50,0.97); backdrop-filter: blur(12px); box-shadow: 0 2px 24px rgba(0,0,0,0.12); }
        ...
        .nav-cta { background: ${B.gold}; color: ${B.forest}; padding: 10px 22px; border-radius: 100px; font-size: 14px; font-weight: 600; text-decoration: none; border: none; cursor: pointer; }
        ...
        /* MOBILE NAV */
        .mobile-menu { display: none; position: fixed; top: 60px; left: 0; right: 0; background: rgba(27,67,50,0.98); backdrop-filter: blur(12px); padding: 20px; z-index: 99; }
        .mobile-menu a { display: block; color: white; text-decoration: none; padding: 14px 0; font-size: 16px; border-bottom: 1px solid rgba(255,255,255,0.1); }
        ...
        .hero { min-height: 100vh; display: flex; align-items: center; justify-content: center;
          background: linear-gradient(135deg, ${B.forest} 0%, ${B.dark} 40%, ${B.green} 100%);
          padding: 100px 20px 60px; text-align: center; }
        .hero-inner { max-width: 800px; }
        .hero-logo { height: 80px; width: auto; margin-bottom: 28px; }
        ...
        .hero h1 span { color: ${B.mint}; }
        .hero p { font-size: 18px; color: rgba(255,255,255,0.8); line-height: 1.6; max-width: 580px; margin: 0 auto 36px; font-weight: 300; }
        .hero-cta { display: inline-flex; align-items: center; gap: 10px; background: ${B.gold}; color: ${B.forest};
          font-size: 16px; font-weight: 600; padding: 16px 36px; border-radius: 100px; text-decoration: none; transition: all 0.3s; }
        .hero-cta:hover { background: #D4B87A; transform: translateY(-2px); box-shadow: 0 8px 30px rgba(201,169,110,0.35); }
        ...
        .section-title { font-family: 'Playfair Display', serif; font-size: 42px; font-weight: 600; color: ${B.forest}; margin-bottom: 14px; }
        .section-sub { font-size: 16px; color: #6B7C6D; line-height: 1.7; max-width: 560px; margin-bottom: 48px; }
        ...
        .svc-card { background: white; border: 1px solid #E8EDE9; border-radius: 14px; padding: 28px 24px; transition: all 0.3s; }
        .svc-card:hover { transform: translateY(-3px); box-shadow: 0 12px 40px rgba(27,67,50,0.08); }
        ...
        .svc-name { font-family: 'Playfair Display', serif; font-size: 20px; font-weight: 600; color: ${B.forest}; margin-bottom: 10px; }
        .svc-desc { font-size: 14px; color: #6B7C6D; line-height: 1.6; }
        ...
        .step-num { font-family: 'Playfair Display', serif; font-size: 40px; font-weight: 700; color: rgba(255,255,255,0.08); margin-bottom: 12px; }
        .step-title { font-family: 'Playfair Display', serif; font-size: 18px; font-weight: 600; color: ${B.mint}; margin-bottom: 8px; }
        ...
        .health-num { font-family: 'Playfair Display', serif; font-size: 36px; font-weight: 700; color: ${B.forest}; margin-bottom: 6px; }
        .health-label { font-size: 13px; color: #6B7C6D; line-height: 1.5; }

        @media (max-width: 768px) { .health-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 480px) {
          .health-grid { grid-template-columns: 1fr; }
          .health-num { font-size: 32px; }
        }

        /* AREAS */
        .areas-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 28px; }
        .area-label { font-weight: 600; color: ${B.forest}; margin-bottom: 10px; font-size: 13px; letter-spacing: 1px; text-transform: uppercase; }
        .pills { display: flex; flex-wrap: wrap; gap: 8px; }
        .pill { background: white; border: 1px solid #E8EDE9; border-radius: 100px; padding: 8px 18px; font-size: 13px; font-weight: 500; color: ${B.dark}; }

        @media (max-width: 768px) { .areas-row { grid-template-columns: 1fr; gap: 24px; } }

        /* CONTACT */
        .contact-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 48px; }
        .form-input { width: 100%; padding: 14px 18px; border: 1.5px solid #E0E5E1; border-radius: 10px;
          font-family: 'DM Sans', sans-serif; font-size: 15px; color: ${B.text}; background: ${B.white}; outline: none; }
        .form-input:focus { border-color: ${B.sage}; }
        .form-input::placeholder { color: #A0ACA2; }
        .submit-btn { background: ${B.forest}; color: white; padding: 14px 28px; border: none; border-radius: 10px;
          font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 600; cursor: pointer; width: 100%; }
        .submit-btn:hover { background: ${B.dark}; }
        ...
        /* FOOTER */
        .footer { background: ${B.charcoal}; color: rgba(255,255,255,0.5); padding: 48px 20px; text-align: center; }
        .footer-logo { height: 40px; margin-bottom: 12px; }
        .footer-tag { font-size: 13px; margin-bottom: 24px; color: rgba(255,255,255,0.4); }
        ...
        <div className="section-inner">
          <div className="contact-grid">
            <div>
              ...
              <p style={{fontSize:15,color:"#6B7C6D",lineHeight:1.7,marginBottom:28}}>
                Tell us about your home and we will respond within 24 hours.
              </p>
              <div style={{display:"flex",flexDirection:"column",gap:20}}>
                <div><strong style={{color:B.forest}}>Phone</strong><br/><span style={{color:"#6B7C6D"}}>(510) 282-8901</span></div>
                <div><strong style={{color:B.forest}}>Email</strong><br/><span style={{color:"#6B7C6D"}}>hello@spotlessblinds.co</span></div>
                <div><strong style={{color:B.forest}}>Hours</strong><br/><span style={{color:"#6B7C6D"}}>Mon - Sat, 8am - 6pm</span></div>
              </div>
            </div>
            <div>
              {submitted ? (
                <div style={{background:B.cream,borderRadius:14,padding:"40px 24px",textAlign:"center"}}>
                  ...
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:B.forest}}>Thank you!</div>
                  <div style={{color:"#6B7C6D",fontSize:14}}>We will be in touch within 24 hours.</div>
                </div>
              ) : (
